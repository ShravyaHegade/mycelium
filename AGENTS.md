...
- Handbook + interactive sandbox live in a separate repo: [mycelium-labs/mycelium-labs.github.io](https://github.com/mycelium-labs/mycelium-labs.github.io) (Pages: https://mycelium-labs.github.io/). Do not re-add a top-level `docs/` or `sandbox/` here.
...
- AF-008 `scope_guard.py`: freeze run tool allowlist; re-check every step; refuse mid-run widen; wraps outside `@loop_guard` / `@ledger`; CLI `mycelium scope status|bind`; YAML `scope_guard:` (opt-in). Entity/path stay on `@bounded`.
- `state_authority.py`: pre-claim superseded-state gate (`state_ref` vs host canonical callback); wraps outside `@scope_guard` / `@loop_guard` / `@ledger`; YAML `state_authority:`; claim stores optional `decision_id` / `state_ref` for audit only.
...
- CLI (`mycelium init|demo|run|transitions|loops|completion|scope|outcomes`) entry is `mycelium/__main__.py`; YAML scaffolds in `mycelium/templates/`; `mycelium/proofs/` + `fixtures/` reproduce langgraph#7417 end-to-end.
